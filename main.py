...
last_month_in_table_DGP = pd.to_datetime(df.dropna(subset=['Реальный ВВП (Темп роста, '
                                                           'квартал к кварталу '
                                                           'прошлого года)']).iloc[
                                             -1]['Целевой показатель']).month
...
update_date_dict = {'invest': 1, 'wage': 1,
                    'DGP': 1, 'unemployment': 1,
                    'trade_turnover': 1, 'Export_import': 1,
                    'IPI': 1, 'inflation': 1}

# Проверяем нужно ли обновлять каждый фактор, если да, flag --> True
if (last_month_in_table_invest + 3 == month_now and update_date_dict['invest'] <= day_now) or \
        (last_month_in_table_invest + 3 < month_now):  # + 3 так как только квартальные данные
    invest_flag = True
if (last_month_in_table_wage + 1 == month_now and update_date_dict['wage'] <= day_now) or \
        (last_month_in_table_wage < month_now):
    wage_flag = True
if (last_month_in_table_DGP + 3 == month_now and update_date_dict['DGP'] <= day_now) or \
        (last_month_in_table_DGP + 3 < month_now):  # + 3 так как только квартальные данные
    DGP_flag = True
if (last_month_in_table_unemployment + 1 == month_now and update_date_dict['unemployment'] <= day_now) or \
        (last_month_in_table_unemployment < month_now):
    unemployment_flag = True
if (last_month_in_table_trade_turnover + 1 == month_now and update_date_dict['trade_turnover'] <= day_now) or \
        (last_month_in_table_trade_turnover < month_now):
    trade_turnover_flag = True
if (last_month_in_table_Export_import + 1 == month_now and update_date_dict['Export_import'] <= day_now) or \
        (last_month_in_table_Export_import < month_now):
    Export_import_flag = True
if (last_month_in_table_IPI + 1 == month_now and update_date_dict['IPI'] <= day_now) or \
        (last_month_in_table_IPI < month_now):
    IPI_flag = True
if (last_month_in_table_inflation + 1 == month_now and update_date_dict['inflation'] <= day_now) or \
        (last_month_in_table_inflation < month_now):
    inflation_flag = True
...
def main_start_parsers():
    ...
    if trade_turnover_flag + unemployment_flag + DGP_flag + wage_flag + invest_flag:
