...
def update_rez_file_X(data, xlsx_path='rez_file_X_v6.xlsx'):
    ...
    if data.values[-1][0] not in list(data_xlsx['date']):
        ...
        data_xlsx = pd.read_excel(xlsx_path)
    name = 'Реальный ВВП (Темп роста, квартал к кварталу прошлого года)'
    for j in data.values:
        data_xlsx.loc[data_xlsx['date'] == j[0], name] = float(j[1].replace(',', '.'))
...
def DGP_parser_main(link_dict):
    ...
    for year in years:
        ...
        if not df_1.empty and not df_2.empty:
            ...
            update_rez_file_X(df_2, xlsx_path='rez_file_X_v6.xlsx')
