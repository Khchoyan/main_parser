...
from help_functions import append_date_rez_file_Y, append_date_rez_file_X, doc_to_docx
...
def update_rez_file_X(data, xlsx_path='rez_file_X_v6.xlsx'):
    ...
    if data.values[-1][0] not in list(data_xlsx['date']):
        ...
        data_xlsx = pd.read_excel(xlsx_path)
    name = ' Инвестиции, % накопленным итогом год к году '
    for j in data.values:
        data_xlsx.loc[data_xlsx['date'] == j[0], name] = j[2]
    data_xlsx.to_excel(xlsx_path, index=False)
...
def run_invest_main(link_dict):
    ...
    update_rez_file_X(new_data, xlsx_path='rez_file_X_v6.xlsx')
