...
last_date_in_table_Nominal_ruble_exchange_rate = pd.to_datetime(df_X.dropna(subset=['Курс Рубля к доллару США, номинальный']).iloc[-1]['date']).month

last_date_in_table_oil_price = pd.to_datetime(df_X.dropna(subset=['Стоимость нефти']).iloc[-1]['date']).month

last_date_in_table_CBR_polls_parser = pd.to_datetime(df_X.dropna(subset=['Мониторинг предприятий ЦБ']).iloc[-1]['date']).month

last_date_in_table_CBR_news_index_parser = pd.to_datetime(df_X.dropna(subset=['Новостной индекс ЦБ']).iloc[-1]['date']).month

last_date_in_table_CBR_invest_parser = pd.to_datetime(df_X.dropna(subset=['Средства клиентов, всего, млн руб.']).iloc[-1]['date']).month

last_date_in_table_CBR_inflation_expectations_parser = pd.to_datetime(df_X.dropna(subset=['Инфляционные ожидания: выросли очень сильно']).iloc[-1]['date']).month

last_date_in_table_retail_turnover_file_X_parser = pd.to_datetime(df_X.dropna(subset=['13.1. Недостаточный платежеспособный спрос']).iloc[-1]['date']).month

last_date_in_table_manufacturer_prices_parser = pd.to_datetime(df_X.dropna(subset=['Индекс цен производителей, в % к предыдущему месяцу']).iloc[-1]['date']).month

last_date_in_table_CBR_key_rate_parser = pd.to_datetime(df_X.dropna(subset=['Ключевая ставка']).iloc[-1]['date']).month

last_date_in_table_CBR_weighted_average_interest_rates_on_loans_parser = pd.to_datetime(df_X.dropna(subset=['Средневзвешенные процентные ставки по кредитам, предоставленным кредитными организациями нефинансовым организациям в рубляхот 1 года до 3 лет']).iloc[-1]['date']).month

last_date_in_table_trade_turnover_dynamics_of_indicators_parser = pd.to_datetime(df_X.dropna(subset=['ДИНАМИКА ОБЪЕМА СТРОИТЕЛЬСТВА']).iloc[-1]['date']).month

last_date_in_table_Freight_tariff_indices_parser = pd.to_datetime(df_X.dropna(subset=['Индексы тарифов на грузовые перевозки Трнаспорт - все виды, в % к предыдущему месяцу']).iloc[-1]['date']).month

last_date_in_table_state_budget_parser = pd.to_datetime(df_X.dropna(subset=['ДИНАМИКА ОБЪЕМА СТРОИТЕЛЬСТВА']).iloc[-1]['date']).month
...
manufacturer_prices_parser_flag = False
CBR_key_rate_parser_flag = False
CBR_weighted_average_interest_rates_on_loans_parser_flag = False
trade_turnover_dynamics_of_indicators_parser_flag = False
Freight_tariff_indices_parser_flag = False
state_budget_parser_flag = False
...
if last_date_in_table_Nominal_ruble_exchange_rate + 1 < month_now:
    Nominal_ruble_exchange_rate_flag = True

if last_date_in_table_oil_price + 1 < month_now:
    oil_price_flag = True

if CBR_polls_parser_flag + 1 < month_now:
    CBR_polls_parser_flag = True

if last_date_in_table_CBR_news_index_parser + 2 < month_now:
    CBR_news_index_parser_flag = True

if last_date_in_table_CBR_invest_parser + 2 < month_now:
    CBR_invest_parser_flag = True

if last_date_in_table_CBR_inflation_expectations_parser + 2 < month_now:
    CBR_inflation_expectations_parser_flag = True

if last_date_in_table_retail_turnover_file_X_parser + 3 < month_now:
    retail_turnover_file_X_parser_flag = True

if last_date_in_table_manufacturer_prices_parser + 1 < month_now:
    manufacturer_prices_parser_flag = True

if last_date_in_table_CBR_key_rate_parser + 1 < month_now:
    CBR_key_rate_parser_flag = True

if CBR_weighted_average_interest_rates_on_loans_parser_flag + 2 < month_now:
    CBR_weighted_average_interest_rates_on_loans_parser_flag = True

if last_date_in_table_trade_turnover_dynamics_of_indicators_parser + 1 < month_now:
    trade_turnover_dynamics_of_indicators_parser_flag = True

if last_date_in_table_Freight_tariff_indices_parser + 1 < month_now:
    Freight_tariff_indices_parser_flag = True

if last_date_in_table_state_budget_parser + 1 < month_now:
    state_budget_parser_flag = True
