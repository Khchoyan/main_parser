...
def electricity_consumption_parser_main(xlsx_path='rez_file_X_v6.xlsx'):
    ...
    data_xlsx = pd.read_excel(xlsx_path)
    date1 = pd.to_datetime(data_xlsx.dropna(subset=['Цены РСВ']).iloc[-1]['date']).strftime('%Y%m%d')
    ...
    update_rez_file_X(rsv_df['CONSUMER_PRICE'], 'Цены РСВ')
    update_rez_file_X(rsv_df['CONSUMER_VOLUME'], 'Объемы РСВ')
    update_rez_file_X(rsv_df[('CONSUMER_PRICE', '1')], 'Цены РСВ ЦЗ 1')
    update_rez_file_X(rsv_df[('CONSUMER_PRICE', '2')], 'Объемы РСВ ЦЗ 1')
    update_rez_file_X(rsv_df[('CONSUMER_VOLUME', '1')], 'Цены РСВ ЦЗ 2')
    update_rez_file_X(rsv_df[('CONSUMER_VOLUME', '2')], 'Объемы РСВ ЦЗ 2')
